...
async function postData() {
  try {
    ...
    // === SMTP TOKEN ===
    // info@furien.jp
    const SECURE_TOKEN = '6b4f90c6-5b7b-40ba-8dd3-b61a8923847f'

    // === 送信先 ===
    const DELIVERY_ADDRESS = 'soichiro.mamiya@anconsulting.jp'

    // === 送信元 ===
    const SENDER_ADDRESS = 'info@furien.jp'
      ...
      reader.onload = async function (e) {
        ...
        if (processedFiles === data.value.length) {
          ...
          const response = await Email.send({
            ...
            To: DELIVERY_ADDRESS,
            From: SENDER_ADDRESS,
            Subject: '【Marketo】ファイルアップロード通知',
            Body: `■ファイルのアップロードがありました。<br><br>Marketo要員ページ:<br>${
              mktoId
                ? 'https://app-ab54.marketo.com/leadDatabase/loadLeadDetail?leadId=' + mktoId
                : 'IDが取得できませんでした。'
            }<br><br>Email:<br>${email}<br><br>ファイル:<br>${data.value
          ...
          if (response == 'OK') {
            if (email !== 'none' && email) {
              // emailがある場合はマルケトのフォームパーツで遷移先がきまる
              let values = {
                Email: email,
                FAC_sheet: data.value.map((file) => file.filename).join(';')
              }
              // eslint-disable-next-line no-undef
              ...
              console.error('Could not retrieve the email. The data will not be sent to Marketo.')
...
<template>
  <form @submit.prevent="submitForm">
    ...
    <div class="error" v-else-if="status === 'error'">
      ...
      <p>
        お手数ですが、お送りしているメール（ccg@anconsulting.jp）のご返信にて、
        <br />
        ファイルを送付していただけると幸いです。
      </p>
    </div>
